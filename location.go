...
func main() {
	...
	fmt.Println(location, err, "time.LoadLocation(", googleTimezone.TimezoneID, ")")

	{
		location, err := time.LoadLocation("EST")
		l, r := time.LoadLocation(googleTimezone.TimezoneID)
		fmt.Println(location, err, l, r, "time.LoadLocation(", googleTimezone.TimezoneID, ")")
	}
	if err != nil {
		fmt.Printf("ERROR : %s", err)
		return
...
func RetrieveGoogleTimezone(latitude float64, longitude float64) (googleTimezone *GoogleTimezone, err error) {
	...
	// ioutil.WriteFile("location.json", rawDocument, 0644)
